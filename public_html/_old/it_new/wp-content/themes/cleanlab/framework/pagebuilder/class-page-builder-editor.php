...
class ZnPageBuilderEditor  {
	...
	function zn_add_factory(){
		...
		<script>
			...
			!function ($) {
				$.ZnPbFactory = {
					current_layout : <?php echo json_encode( $this->get_current_layout_opt_map() ); ?>
				}
			}(jQuery)
	...
	function get_current_layout_opt_map(){

		$layout_data  = ZN()->pagebuilder->current_page_layout;
		return $this->build_options_array( $layout_data );
	...
	function before_element( $element ) {
		...
		echo '<div class="zn_pb_el_container zn_pb_section '.$size.' zn_element_'.strtolower($element->info['class']).'" data-form-uid="'.$uid.'" data-el-name="'.$element->info['name'].' options" data-uid="'.$element->data['uid'].'" data-level="'.$element->info['level'].'" data-object="'.$element->info['class'].'">';
